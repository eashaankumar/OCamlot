...
function ts_resortTable(lnk) {
    ...
    for (var i= 0; i <= td.cellIndex; i++) {
        if (tr.cells[i].colSpan) {
            if (tr.cells[i].id == "gradeheader") column += numprobs;
    ...
    switch (sortType) {
        ...
        case "prob_score_cell": sortfn= ts_sort_scores;             break;
    ...
    if (span.getAttribute("sortdir") == 'down') {
        ...
        var reverseSort = function (x, y) {
            var orig = sortfn(x, y);
            return -orig;
        };
        newRows = stableSort(newRows, reverseSort);
        ARROW = '&nbsp;&nbsp;';
        span.setAttribute('sortdir','up');
    } else {
        setCookie(table.id + '_sortdir', 'up')
        upDown=1; // for score-sort
        newRows = stableSort(newRows, sortfn);
        ARROW = '&nbsp;&nbsp;';
    ...
    for (i=0;i<newRows.length;i++) {
    	if (!newRows[i].className || (newRows[i].className && (newRows[i].className.indexOf('sortbottom') == -1))) {
    		var newClass = (i % 10 < 5) ? "row_even" : "row_odd";
    		if (newRows[i].className.indexOf('row_even') != -1) newRows[i].className = newRows[i].className.replace("row_even", newClass);
    		if (newRows[i].className.indexOf('row_odd') != -1) newRows[i].className = newRows[i].className.replace("row_odd", newClass);
